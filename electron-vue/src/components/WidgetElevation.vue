...
const your_color = "#3e92cc";
const opponent_color = "#d36135";

let marker_you, marker_opponent, path;
...
export default {
    ...
    mounted () {
        ...
        const svg = d3.select("#widget-elevation")
            .append("div")
        ...
        marker_you = svg.append("circle")
            .attr("id", "marker-you")
            .attr("r", 5)
            .attr("fill", your_color)
            .attr("transform", "translate(0,0)");

        marker_opponent = svg.append("circle")
            .attr("id", "marker-opponent")
            .attr("r", 5)
            .attr("fill", opponent_color)
            .attr("transform", "translate(0,0)");
    },
    watch: {
        distanceYou: function (val) {
            if (marker_you && path) {
                let co = path.getPointAtLength((val * path.getTotalLength()) / this.race.totalDistance);
                marker_you.attr("transform", "translate(" + co.x + "," + co.y + ")");
            }
        },
        distanceOpponent: function (val) {
            if (marker_opponent && path) {
                let co = path.getPointAtLength((val * path.getTotalLength()) / this.race.totalDistance);
                marker_opponent.attr("transform", "translate(" + co.x + "," + co.y + ")");
            }
        }
    }
};
